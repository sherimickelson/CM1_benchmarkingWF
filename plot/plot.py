...
def plot_it(json_file):
    ...
    print("control log file: "+fns["control"])
    print("experiment log files:")
    for f in sorted(fns["experiment"].keys()):
        print(f)
    print("\n")
    ...
    for p in plots:
        ...
        values[p][fns["control"]]=OrderedDict()
        for fn in sorted(fns["experiment"].keys()):
...
#            print(values[p][fn])
    ...
    def plot(_plt, i):
        colors = []
        for fn in sorted(fns["experiment"].keys()):
            res = {key: (values[plots[i]][fns["control"]][key]-values[plots[i]][fn][key])/values[plots[i]][fns["control"]][key] for key in values[plots[i]][fns["control"]].keys()}
            if 'auto' in fns["experiment"][fn]:
                colors.append(_plt.plot(values[plots[i]][fn].keys(), res.values())[0].get_color())
            else:
                colors.append(_plt.plot(values[plots[i]][fn].keys(), res.values())[0].get_color())
    ...
    fig.legend(handles=lines, labels=sorted(fns["experiment"].keys()), loc = "lower center")
